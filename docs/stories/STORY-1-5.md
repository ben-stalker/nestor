...
- [x] Migration `server/migrations/001_app_settings.sql` creates `app_settings(key TEXT PRIMARY KEY, value TEXT, updated_at INTEGER NOT NULL)`
- [x] `server/src/repositories/BaseRepository.ts` is an abstract class holding a `Database` reference and exposing protected helpers `get<T>(sql, params)`, `all<T>(sql, params)`, `run(sql, params)`
- [x] `AppSettingsRepository` extends `BaseRepository` and exposes `getAll(): Record<string, unknown>`, `get<T>(key: string): T | undefined`, `set(key, value)`, `setMany(map)`, `delete(key)`
- [x] Settings are JSON-serialised on write and parsed on read
- [x] An in-memory cache is populated on first `getAll()` and invalidated on every write
- [x] Type safety: `server/src/db/settings-keys.ts` declares Zod schemas for each known key (e.g. `location`, `orientation`, `enabled_nav_modes`, `meal_slots`, `quiet_hours`, `kiosk_lock`, `setup_complete`, `encryption_salt`, `language`, `locale`, `voice_internal_token`, `update_available_version`, `reminder_windows`, `fuel_rates`, `community_plugin_index_url`, `vehicle_reminder_days`, `vaccination_schedule_region`, `nav_layout`, `plugins_enabled`)
- [x] Strict typed accessor: `appSettings.get<typeof LocationSchema>('location')` returns the parsed type
- [x] Unit tests cover get/set, batch setMany, cache hit/miss, cache invalidation, JSON round-trip, unknown-key warning
...
- [x] Unit: `set('foo', { a: 1 })` → `get('foo')` returns `{ a: 1 }` (object identity not preserved, equality yes)
- [x] Unit: `setMany({ a: 1, b: 2 })` writes both atomically — failing one rolls back both
- [x] Unit: cache: 1st `getAll()` reads from DB, 2nd reads from cache; verify by spying on the prepared statement
- [x] Unit: any `set()`/`setMany()`/`delete()` invalidates the cache
- [x] Unit: Zod-validated key with bad value via `validateSetting` throws
- [x] Unit: round-trip primitives: string, number, boolean, array, nested object
